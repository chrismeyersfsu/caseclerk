...
def test_file_overrides_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    path.write_text(json.dumps({"clioRoot": "/mnt/clio", "processing": {"concurrency": 4}}))

    cfg = load_config()
    assert cfg.clio_root == "/mnt/clio"
...
def test_env_overrides_file(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CASECLERK_CLIO_ROOT", "/mnt/env-wins")
    ...
    cfg = load_config()
    assert cfg.clio_root == "/mnt/env-wins"
